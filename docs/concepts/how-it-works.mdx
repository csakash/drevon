...
```mermaid
graph TD
    A[Agent reads INDEX.md] --> B[Agent works on your task]
    B --> C[Agent writes learnings to memory]
    C --> D[Agent creates reusable prompts]
    D --> A
```

1. **Session start** — agents read only `INDEX.md` to restore context, then load individual topic files on demand
